...
func MakeSaveButton(update bool) string {
	if update {
		return `<button id='btnSave' type='submit' aria-busy='false' class='outline secondary' form='theForm' title='Save Record'>` + svg.GetIcon("save") + `</button><input type='hidden' id='canSave' value='1'>`
	}
...
func MakeDeleteButton(delete bool) string {
	if delete {
		return `<button id='btnDelete' title='Delete Record' class='outline secondary' data-target="delete-dialog" onclick='deleteRecord(event);'>` + svg.GetIcon("delete") + `</button><input type='hidden' id='canDelete' value='1' >`
	}
